...
fair <- function(Y, Z, X = NULL, H, K = 1, asym = FALSE, maxvarlag = 12, varci = FALSE, varboot = 500, spanloess = 0.5, sdpar = 10, cores = NA, chains = 4, iter = 2000, adapt_delta = 0.95, max_treedepth = 12, timeout = 180) {
  start_time <- Sys.time() 
    ...
    # Running Stan
        # Forcing the initial stan guess to be Gaussians computed using priors
          get_inits <- function(chain_id) {
            if(K==1){
              list(a1 = priors$a, b1 = priors$b, c1 = priors$c, sigma = 1)
            } else {
              list(
                a1 = priors$a1, 
                b1 = priors$b1, 
      ...
      # Function
        run_fair <- function(stan_file, data_list, chains, iter, cores, timeout, inits) {
          R.utils::withTimeout(
            stan(
              file = stan_file,
              data = data_list,
      ...
      # Run FAIR
        if (K == 1) {
          stan_file <- "FAIRone.stan"
        } else {
          stan_file <- "FAIRtwo.stan"
        }
        
        if (asym == FALSE) {
          message(paste0("Estimating symmetric FAIR(", K, ")"))
          fit <- run_fair(stan_file, sdata, chains, iter, cores, timeout, inits)
        } else {
          message(paste0("Estimating FAIR(", K, ") for positive shocks"))
          pfit <- run_fair(stan_file, pdata, chains, iter, cores, timeout, inits)
          
          message(paste0("Estimating FAIR(", K, ") for negative shocks"))
          nfit <- run_fair(stan_file, ndata, chains, iter, cores, timeout, inits)
        }
    ...
    # Assemble the final output list
      if (!asym) {
        ...
        output <- list(
          ...
          negative.stan = summary(pfit),
          positive.stan = summary(nfit),
          data = list(
            Y = Y,
            Z = Z,
            X = X
